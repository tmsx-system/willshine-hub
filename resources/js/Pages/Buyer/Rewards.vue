...
const tierMilestones = [
    ...
    { name: 'Silver', points: 500, icon: Award },
    { name: 'Gold', points: 1000, icon: Star },
    { name: 'Platinum', points: 2500, icon: Gem },
];
...
<template>
    <div class="px-4 sm:px-6 lg:px-8 py-6 max-w-7xl mx-auto space-y-6">
            ...
            <div class="relative grid sm:grid-cols-2 gap-6 items-center">
                <div>
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        <component :is="tierIcon[tier] ?? Star" class="h-7 w-7 text-white" />
                        <div>
                            <p class="text-white font-bold text-base">Tier {{ tier }}</p>
                            <p class="text-pink-200 text-xs">Poin dan benefit mengikuti data reward customer.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white/15 backdrop-blur rounded-2xl p-5 border border-white/20">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <component :is="tierIcon[next_tier] ?? Gem" class="h-5 w-5 text-white" />
                    </div>
                    <div class="progress-track bg-white/20">
                        <div
                            class="progress-fill"
                            :style="{ width: Math.min(100, next_points > 0 ? (points / next_points) * 100 : 0) + '%' }"
                        />
                    </div>
                    ...
                    <p class="mt-2 text-center text-white text-xs font-semibold">
                        Kurang <span class="text-yellow-300">{{ Math.max(0, next_points - points).toLocaleString('id-ID') }} poin</span> lagi
                    </p>
                </div>
            </div>
                ...
                <div class="relative">
                    ...
                    <div class="grid grid-cols-4 relative">
                        <div v-for="milestone in tierMilestones" :key="milestone.name" class="flex flex-col items-center gap-1.5">
                            <div
                                :class="['w-8 h-8 rounded-full flex items-center justify-center z-10 transition-all duration-300', points >= milestone.points ? 'bg-white shadow-lg scale-110 text-[#BE185D]' : 'bg-white/20 text-white/50']"
                            >
                                <component :is="milestone.icon" class="h-4 w-4" />
                            </div>
        ...
        <div class="card p-1.5 flex gap-1 animate-fade-in-up delay-100">
            ...
            >
                <Gift class="mr-1 inline h-4 w-4" />
                Tukar Reward
            </button>
            <button
            ...
            >
                <History class="mr-1 inline h-4 w-4" />
                Riwayat Poin
            </button>
        </div>

        <div v-if="activeTab === 'rewards' && rewards.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
            <div
            ...
            >
                <div class="flex items-start justify-between gap-3">
                    <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm">
                        <component :is="rewardCategoryIcons[reward.category] ?? Gift" class="h-6 w-6 text-pink-600" />
                    </div>
        ...
        <div v-else-if="activeTab === 'rewards'" class="card py-16 px-6 text-center animate-scale-in">
            <h3 class="text-lg font-bold text-gray-800">Belum ada reward aktif</h3>
            <p class="mt-2 text-sm text-gray-500">Reward akan tampil setelah data tersedia dari backend.</p>
        </div>
        ...
        <div v-else class="card overflow-hidden animate-fade-in">
            ...
            <div v-if="history.length" class="divide-y divide-gray-50">
                <div
                ...
                >
                    ...
                    >
                        <ArrowUp v-if="item.type === 'earn'" class="h-5 w-5 text-emerald-600" />
                        <Gift v-else-if="item.type === 'redeem'" class="h-5 w-5 text-pink-600" />
                        <Star v-else class="h-5 w-5 text-amber-600" />
                    </div>
                    ...
                    <div class="text-right flex-shrink-0">
                        <p :class="['text-base font-extrabold', item.points > 0 ? 'text-emerald-600' : 'text-pink-600']">
            ...
            <div v-else class="px-6 py-14 text-center">
                <h3 class="text-base font-bold text-gray-800">Belum ada riwayat poin</h3>
                <p class="mt-2 text-sm text-gray-500">Transaksi poin customer akan tampil di sini.</p>
            </div>
        </div>
    </div>
</template>
